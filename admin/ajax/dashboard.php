<?php 
  ...
  if(isset($_POST['booking_analytics']))
  {
    ...
    $result = mysqli_fetch_assoc(mysqli_query($con,"SELECT 
      ...
      SUM(CASE WHEN booking_status!='pending' AND booking_status!='payment failed' THEN `final_amt` END) AS `total_amt`,

      COUNT(CASE WHEN booking_status='confirmed' AND arrival=1 THEN 1 END) AS `active_bookings`,
      SUM(CASE WHEN booking_status='confirmed' AND arrival=1 THEN `final_amt` END) AS `active_amt`,

      COUNT(CASE WHEN booking_status='cancelled' AND refund=0 THEN 1 END) AS `cancelled_bookings`,
      SUM(CASE WHEN booking_status='cancelled' AND refund=0 THEN `final_amt` END) AS `cancelled_amt`
